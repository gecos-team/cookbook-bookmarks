...
action :add do
  if File.exist? new_resource.sqlitedb
    ...
    if url.nil?
      ...
      if id_folder_bookmarks.nil?
        db.execute("INSERT INTO moz_bookmarks
                    ...
                    VALUES
                    (2,#{id_toolbar_bookmakrs},#{last_pos_toolbar+1},\'Marcadores corporativos\',#{date_now},#{date_now})")
      ...
      db.execute("INSERT INTO moz_places
                  (url,title,rev_host,visit_count,hidden,type,last_visit_date)
                  VALUES
